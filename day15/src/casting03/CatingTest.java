...
//		animal2.walk();
	...
	//1. 메소드의 매개변수로 여러 타입으 ㅣ값을 받는 경우
	// 일반적으로는 가각의 타입을 처리하는 메소드를  별도로 만들어야한다
	// 매개변수에 업캐스팅을 하게 되면 1개의 타입으로 자식 클래스의값들을 담아줄 수 있다
	
	//2. 같은 부모 클래스가 존재한다면 매개변수를 부모 타입으로 받는다(업캐스팅)
	// 하나의 메소드에서 매개변수를 부모타입으로 만들면 여러 자식 타입을 ㅂ다아서 사용이 가능하다(다형성)
	...
	static void tryWalk(Animal ani) {
		//매개변수로 받아노 객체의 주소값이 Cat 타입인지, Dog 타입인지 확인한다
